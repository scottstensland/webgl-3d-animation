...
let aggregate_buffer_size = 0;

let desired_synth_buff_size;
let curr_sampled_buffer;
let synth_from_sample_buffer; // actual audio curve NOT for display
let elephant_buffer;
let running_sampled_offset = 0; // count in bytes size buffer already set into sampled buffer storage

let continue_sampling = false;

export const animals_synth = {}; // to display synthesized buffer
export const animals_sampled = {}; // to display sampled buffer

const synth_object = {};

let gl; // handle given from calling context

// let suggested_max_num_control_points = 10;
// let curr_control_point;
// let DIM_CONTROL_POINTS = 3;

export const X = 0; // index into DIM_CONTROL_POINTS - each control points has XY and MASS
export const Y = 1;
export const Z = 2;
// let MASS = 3;
...
// let control_point_buffer = new Float32Array(suggested_max_num_control_points * DIM_CONTROL_POINTS);
...
let curr_3D_array = []; // each NON 0.0 element says to rotation cylinder about this axis by this radians
...
// let SIZE_DIM_2D = 2;	// X and Y
export const SIZE_DIM_3D = 3;
export const SIZE_DIM_COLORS = 4;

let is_synth_ready_to_display = false;

let was_a_sample_taken = false;

// ---

export function get_size_sampled_buffer() {
  return desired_synth_buff_size;
}

export function get_sampled_buffer() {
...
function pop_seed_buff(seed_obj) {
  const size_seed = seed_obj.size;
  const seed_buffer = seed_obj.buffer;

  const min_boundary = -1.0;
  const max_boundary = 1.0;
  const size_swiggle = 0.2;

  let curr_y = 0.0;
  let tmp_y;

  for (let index = 0; index < size_seed; index++) {
...
*/
  ...
  const num_channels = buff_obj.num_channels;

  console.log('inside pop_stradivarius with     num_channels ', num_channels);

  // let desired_frequency = 800.0;	// Hertz == cycles per second
  // let desired_frequency = 3600.0;	// Hertz == cycles per second
  const desired_frequency = buff_obj.frequency; // Hertz == cycles per second

  // let sample_rate = 44100;	// samples_per_second
  const sample_rate = buff_obj.sample_rate; // samples_per_second
...
*/
  ...
  const radians_per_sample = (2.0 * Math.PI * desired_frequency) / sample_rate;

  console.log('radians_per_sample ', radians_per_sample);
  // new radians_per_sample  0.11398068584452764

  const synth_mode = 1; // 1 == generate sin wave
  // let synth_mode = 2;	// 2 == elephant roar
  ...
  let curr_y;
  let theta = 0.0;
  const incr_theta = radians_per_sample;
...
function genetic_synth(buff_obj) {
  ...
  for (const property in buff_obj) {
...
let is_elephant_roar_done = false;
...
function elephant_roar(buff_obj) {
  ...
  */
  ...
  if (is_elephant_roar_done) {
    ...
    console.log(
      "Let's create an elephant roar !!!!!!! ... desired_synth_buff_size",
...
function count_properties(given_obj) {
  let count = 0;
  for (const curr_property in given_obj) {
...
export function get_buffer(given_flavor) {
  //		get_synth_from_sample_buffer

  const chosen_buff_obj = {}; // object to which we store :  buffer, frequency, sample_rate, bit_depth
...
let flag_trim_sampled_buffer_to_match_source_size = false;

export function take_another_sample(when_sample_is_done_callback, given_mode) {
...
export function init_audio_processing(given_gl, BUFF_SIZE) {
  // 
  ...
  // let sample_synth_buff_size_multiplier = 1; // size of synth/sampled buff as multiple of source buff size
  const sample_synth_buff_size_multiplier = 5; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 10; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 20; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 50; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 100; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 250; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 500; // size of synth/sampled buff as multiple of source buff size
  // let sample_synth_buff_size_multiplier = 1000; // size of synth/sampled buff as multiple of source buff size
...
function show_buffer(given_audio_buffer, given_buffer_size, limit_to_see) {
  ...
  const max_visible_index = given_buffer_size > limit_to_see ? limit_to_see : given_buffer_size;

  for (let curr_index = 0; curr_index < max_visible_index; curr_index++) {
...
let seeing_zero_value = false;
let index_when_zero_value_started = 0;
let count_num_zero_seen_consecutive = 0;
let max_count_num_zero_seen_consecutive = 0;
...
function synth_audio_curve_pop_buff(given_sampled_buffer, d_sample_buff, d_synth_buffer, buffer_size, percent_probed) {
  ...
  if (100 != percent_probed) {
    const err_msg = "ERROR - currently only handles 100% sampling";

    console.log(err_msg);
    alert(err_msg);
  }

  const synth_mode_monte_carlo = 1;
  const synth_mode_half_split = 2;

  let curr_x = 0.0,
    curr_y = 0.0,
    curr_z = 0.0;

  // let max_size_incr_y = 1.0 / 10.0;
  // let max_size_incr_y = 1.0 / 8.0;
  // let min_size_incr_y = -1.0 * max_size_incr_y;
  ...
  let max_y_error = 0.001; // make this smaller for more accurate half splitting probe discovery of actual y
  // let max_y_error = 0.005;	// make this smaller for more accurate half splitting probe discovery of actual y
  // let max_y_error = 0.005;	// make this smaller for more accurate half splitting probe discovery of actual y
  let max_trials = 8; // maximum number of iterations of below probe to half split distance until we find y
  ...
  let count_limiting_condition_error = 0; // running total to see how many iterations due to error reached
  let count_limiting_condition_count = 0; // how many due to maxxing out of permitted iterations

  let synth_mode = synth_mode_monte_carlo;
  // let synth_mode = synth_mode_half_split;

  // let best_y_so_far_error_distance = Number.MAX_VALUE;
  let best_y_so_far_error_distance = 999.99;
  let best_y_so_far;

  let curr_abs_error_distance;
  ...
  if (max_count_num_zero_seen_consecutive == buffer_size) {
    console.log('NOTE - seeing ALL zeros buffer_size == max_count_num_zero_seen_consecutive ', max_count_num_zero_seen_consecutive);
  }
} // synth_audio_curve_pop_buff

export function get_display_ready_flag() {
...
function synth_curve(sampled_buffer, given_buffer_size) {
  const percent_probed = 100; // percentage of x values in source sampled curve recreated in synth
...
function process_sampled_audio(sampled_buffer, aggregate_buffer_size) {
  if (sampled_buffer.length != aggregate_buffer_size) {
    const err_msg = "ERROR - mismatch on sampled buffer size and given size";
...
export function perform_sampling(given_audio_buffer, given_buffer_size, providence) {
  ...
  if (continue_sampling && aggregate_buffer_size >= desired_synth_buff_size) {
    ...
    if (aggregate_buffer_size != desired_synth_buff_size) {
      const err_msg = 'ERROR - desired_synth_buff_size is NOT multiple of aggregate_buffer_size';
      console.log(err_msg);
      alert(err_msg);
    }
